//tried to take input (no use for now)
function takeInput(inputType) {
  process.stdin.resume();
  process.stdin.setEncoding("utf8");

  process.stdin.on("data", function (data) {
    const input = data.trim(); // Remove any leading/trailing spaces

    if (inputType == "number") {
      const numbers = input.split(" ").map(Number); // Split by space and convert to integers
      chooseProgram(numbers);
    } else if (inputType == "string") {
      const string = input.split(" ").map(String); // Split by space and convert to strings
    }
  });
}

function chooseProgram(programInput) {
  let programNumber = programInput.flat();

  console.log(programNumber[0], typeof programNumber[0]);
  switch (programNumber[0]) {
    case 1:
      var inputInteger = takeInput("number");
      rectangles(inputInteger);
      break;
    default:
      console.log("Please choose from above programs only");
      break;
  }
}
